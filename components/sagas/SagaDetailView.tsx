    ...
    >
      {isDone ? (
        <div className="absolute -top-3 left-1/2 z-10 flex -translate-x-1/2 gap-0.5">
          {[1, 2, 3].map((star) => (
            <Star
              key={star}
              className="h-4 w-4 fill-neo-yellow stroke-foreground stroke-[2]"
            />
          ))}
        </div>
      ) : null}
      ...
      <div
        className={cn(
          ...
          !isLocked && 'motion-safe:group-hover:-translate-y-1 motion-safe:group-hover:-translate-x-[2px]'
        )}
      >
            ...
            <section key={chapter.id} className="mb-16">
              ...
              <div className="relative flex flex-col items-center gap-12">
                ...
                {chapter.lessons.map((lesson, lessonIndex) => {
                  ...
                  return node ? <NodeBubble key={lesson.id} sagaId={saga.id} node={node} /> : null;
                })}
              </div>
            </section>
          );
